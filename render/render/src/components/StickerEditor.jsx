...
const StickerEditor = ({
  ...
  // Sync with parent stickers
  useEffect(() => {
    setStickers(initialStickers);
  }, [initialStickers]);

  // Sync with parent selected sticker
  useEffect(() => {
  ...
  const handleMouseDown = useCallback((e, stickerId, action = 'move') => {
    ...
    if (action === 'resize') {
      ...
      const stickerCenterX = (sticker.x / 100) * (displayDimensions.width / zoom);
      const stickerCenterY = (sticker.y / 100) * (displayDimensions.height / zoom);
  ...
  return (
    <div className="w-full h-screen bg-zinc-900 text-zinc-100 flex flex-col overflow-hidden font-sans antialiased">
      <header className="flex items-center justify-between h-14 px-4 bg-gradient-to-b from-zinc-900/90 to-zinc-900/60 border-b border-zinc-800 z-20">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-zinc-800/40 rounded-md px-2 py-1">
            <button onClick={() => { setLeftPanelOpen(true); setTool('select'); }} className="p-2 rounded hover:bg-zinc-800 text-zinc-200 flex items-center gap-2" title="Open Tools">
              <Layers className="w-4 h-4" />
              <span className="text-xs text-zinc-300 hidden sm:inline">Tools</span>
            </button>
            <button onClick={() => { setRightPanelOpen(true); setTool('select'); }} className="p-2 rounded hover:bg-zinc-800 text-zinc-200 flex items-center gap-2" title="Open Properties">
          ...
          <div className="flex items-center gap-2 bg-zinc-800/60 rounded-md px-2 py-1">
            <button onClick={() => setTool('select')} className={`p-1 rounded ${tool === 'select' ? 'bg-zinc-700 text-cyan-300' : 'text-zinc-300 hover:text-white'}`} title="Select (V)">
              <MousePointer2 className="w-4 h-4" />
            </button>
            <button onClick={() => setTool('hand')} className={`p-1 rounded ${tool === 'hand' ? 'bg-zinc-700 text-cyan-300' : 'text-zinc-300 hover:text-white'}`} title="Hand (H)">
              <Hand className="w-4 h-4" />
            </button>
          </div>

          <button onClick={() => setShowGrid(!showGrid)} className={`p-2 rounded-md ${showGrid ? 'bg-zinc-800 text-cyan-300' : 'bg-transparent text-zinc-300 hover:bg-zinc-800'}`} title="Toggle Grid">
            <Grid3x3 className="w-4 h-4" />
          </button>

          <div className="flex items-center gap-2 bg-zinc-800/40 rounded-md px-2 py-1">
            <button onClick={() => setZoom(Math.max(0.25, zoom - 0.25))} className="p-1 text-zinc-300"><ZoomOut className="w-4 h-4" /></button>
            <div className="text-xs font-mono text-zinc-300 w-14 text-center">{Math.round(zoom * 100)}%</div>
            <button onClick={() => setZoom(Math.min(3, zoom + 0.25))} className="p-1 text-zinc-300"><ZoomIn className="w-4 h-4" /></button>
          </div>

          <div className="flex items-center gap-2">
            {onCancel && (
              <button onClick={onCancel} className="px-3 py-1.5 rounded-md bg-zinc-700 hover:bg-zinc-600 text-white text-sm">
                Cancel
              </button>
            )}
            {onApply && (
              <button onClick={onApply} disabled={isApplying} className="px-3 py-1.5 rounded-md bg-cyan-600 hover:bg-cyan-500 text-white text-sm disabled:opacity-60">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {isMobile && (leftPanelOpen || rightPanelOpen) && (
          <div onClick={() => { setLeftPanelOpen(false); setRightPanelOpen(false); }} className="fixed inset-0 bg-black/40 z-30" />
        )}

        {leftPanelOpen && (
          <aside className={`z-40 ${isMobile ? 'fixed left-0 top-14 bottom-0 w-80' : 'w-72'} bg-zinc-900 border-r border-zinc-800 shadow-lg flex flex-col`}>
            ...
            <div className="p-3 overflow-auto space-y-3 flex-1">
              <div className="space-y-2">
                <button onClick={() => setTool('select')} className={`w-full text-left px-3 py-2 rounded-md ${tool === 'select' ? 'bg-zinc-800 text-cyan-300' : 'hover:bg-zinc-800 text-zinc-300'}`}>
                ...
                <button onClick={() => setTool('move')} className={`w-full text-left px-3 py-2 rounded-md ${tool === 'move' ? 'bg-zinc-800 text-cyan-300' : 'hover:bg-zinc-800 text-zinc-300'}`}>
                ...
                <button onClick={() => setTool('scale')} className={`w-full text-left px-3 py-2 rounded-md ${tool === 'scale' ? 'bg-zinc-800 text-cyan-300' : 'hover:bg-zinc-800 text-zinc-300'}`}>
                ...
                <button onClick={() => setTool('rotate')} className={`w-full text-left px-3 py-2 rounded-md ${tool === 'rotate' ? 'bg-zinc-800 text-cyan-300' : 'hover:bg-zinc-800 text-zinc-300'}`}>
        ...
        {rightPanelOpen && (
          <aside className={`z-40 ${isMobile ? 'fixed right-0 top-14 bottom-0 w-80' : 'w-80'} bg-zinc-900 border-l border-zinc-800 shadow-lg flex flex-col`}>
            <div className="flex items-center justify-between px-3 h-12 border-b border-zinc-800">
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 md:hidden z-40 flex items-center gap-3">
        <button onClick={() => setLeftPanelOpen(true)} className="p-3 rounded-full bg-zinc-800 text-zinc-200 shadow">
          <Layers className="w-4 h-4" />
        </button>
        <button onClick={() => setRightPanelOpen(true)} className="p-3 rounded-full bg-zinc-800 text-zinc-200 shadow">
          <Settings className="w-4 h-4" />
        </button>
        <button onClick={() => onApply?.()} className="p-3 rounded-full bg-cyan-600 text-white shadow">
          <Settings className="w-4 h-4" />
        </button>
      </div>
    </div>
